...
def measure_method(data_noised, method,
                   labels,
                   data_name,
                   subsample_idx=None):
    ...
    ari_score = demap.demap.ARI(
        labels, embedding, subsample_idx=subsample_idx)
...
def measure_all_methods(load_fn,
                        n_cells=None, n_jobs=1,
                        load_params=None, seed=None):
    ...
    if n_cells is not None:
        subsample_idx = np.random.choice(
            data_noised.shape[0], n_cells, replace=False)
    else:
        subsample_idx = None
    measure = partial(measure_method,
                      labels=labels,
    ...
    if n_jobs == 1:
        results = [
            ...
            for method in demap.embed.all_methods]
    else:
...
def measure_splat_range(load_fn, var_name, var_range,
                        n_jobs=1, seed=None, **load_kwargs):
    results = pd.concat([measure_all_methods(
        load_fn,
        load_params=dict(**{var_name: var_value}, seed=seed, **load_kwargs),
        n_jobs=n_jobs, seed=seed)
        for var_value in var_range])
    results.to_csv("../results/ari_s{}_{}_{}_{}_{}.csv".format(
        load_fn.__name__, var_name, var_range.min(), var_range.max(), seed))
    results_agg = results.groupby('method').agg(
        {'ARI': [np.mean, np.std]}).sort_values(('ARI', 'mean'),
                                                  ascending=False)
    print(results_agg)
    return results_agg
...
def run_test(seed, n_jobs=1):
    ...
    measure_splat_range(dataset, metric, metrics[metric],
                        n_jobs=n_jobs, seed=seed)
