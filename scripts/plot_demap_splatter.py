...
metric_titles = {'bcv': 'Biological Variation',
                 'dropout': 'Dropout',
                 'n_cells': 'Subsampled cells',
                 'n_genes': 'Subsampled genes'}
metric_xlabels = {'bcv': 'Coefficient of Variation',
                  'dropout': 'Dropout probability',
                  'n_cells': '# of cells retained',
                  'n_genes': '# of genes retained'}
dataset_ylabels = {'groups': 'Splatter Groups',
                   'paths': 'Splatter Paths'}
method_colors = {'PHATE': 'k',
                 'MDS_on_DM': 'tab:grey',
                 'TSNE_on_DM': 'tab:orange'}
...
def load_all(dataset="paths", metric="bcv"):
    df = pd.concat([pd.read_csv(filename, index_col=0)
                    for filename in glob.glob("../results/{}_{}_*.csv".format(
                        dataset, metric))],
                   sort=True)
...
def summarize_one(dataset="paths", metric="bcv"):
    ...
    df = df.groupby(['method', 'value']).agg({'demap': [np.mean, np.std]}).sort_values(
        ['value', ('demap', 'mean')], ascending=False)
    score = df[('demap', 'mean')].values
    std = df[('demap', 'std')].values
    method, value = list(zip(*df.index.values.tolist()))
    method = np.array(method)
    value = np.array(value)
    return pd.DataFrame(dict(method=method, value=value, mean=score, std=std,
    ...
    for dataset in ['paths', 'groups']:
        for metric in ['dropout', 'bcv', 'n_cells', 'n_genes']:
            ...
            for method in methods:
                plot_idx = (result['method'] == method) & (
                    result['dataset'] == dataset) & (result['metric'] == metric)
                ...
                lines.append(ax.plot(
                    ...
                    label=method, linewidth=3 if method == 'PHATE' else 1,
                    c=map_color(method),
                    linestyle='--' if '_on_' in method else '-')[0])
            ax.set_xticks(x)
...
def plot_all():
    ...
    methods = result.groupby('method').agg({'mean': np.mean}).sort_values(
        'mean', ascending=False).index.values
    ...
    plt.rc('font', size=11)
    fig.set_figwidth(18)
    axes[0, -1].legend(lines, [m.replace("_", " ") for m in methods],
