...
def summarize_one(dataset="paths", metric="bcv"):
    ...
    df = df.groupby(['method', 'value']).agg({'demap': [np.mean, np.std]}).sort_values(
        ('demap', 'mean'), ascending=False)
    score = np.array(["{:.2f} +- {:.2f}".format(m, s) for m, s in zip(
        np.round(df[('demap', 'mean')], 2),
        np.round(df[('demap', 'std')], 2))])
    ...
    for v in unique_inorder(value):
        idx = value == v
        val_result = pd.DataFrame([score[idx]], columns=method[idx],
...
def to_latex(result, filename):
    ...
    with open(filename, 'w') as handle:
        handle.write(result.to_latex().replace(
            "\\$", "$").replace(
            "\\{", "{").replace(
            "\\}", "}").replace(
            "mathbf", "\\mathbf").replace(
            "+-", "\\pm").replace(
            'l' * (result.shape[1] + 1), 'l' + 'r' * result.shape[1]))
