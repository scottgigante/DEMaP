...
def measure_method(data, data_noised, method,
                   data_name,
                   subsample_idx=None):
    ...
    demap_score = demap.DEMaP(
        data, embedding, knn=5, subsample_idx=subsample_idx)
...
def measure_all_methods(data, load_fn,
    ...
    measure = partial(measure_method,
                      data=data, data_noised=data_noised,
                      data_name=data_name,
...
def measure_splat_range(load_fn, var_name, var_range,
    ...
    results_agg = results.groupby('method').agg(
        {'demap': [np.mean, np.std]}).sort_values(('demap', 'mean'),
                                                  ascending=False)
    print(results_agg)
    return results_agg
